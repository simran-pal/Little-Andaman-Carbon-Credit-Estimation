...
| AGBD Map 2020 | AGBD Change 2020-2022 |
|:---:|:---:|
| ![AGBD 2020](figures/agbd_map_2020.png) | ![AGBD 2022](figures/agbd_map_2020.png) |
...
```
Change detection restricted to pixels classified as forest in **both** years to avoid cloud masking artefacts. A 20% uncertainty buffer applied to net credits following Verra AFOLU non-permanence risk guidance.
...
- 2022 cloud cover reduced L2A coverage to 3,561 shots vs 7,722 in 2020
- Mean DeltaAGBD of 15.17 Mg/ha is high relative to typical tropical forest increment - interpret cautiously given 22% model RMSE
...
```
+-- notebooks/
|   +-- carbon_estimation.ipynb   <- full pipeline, single notebook
+-- figures/                      <- maps and charts committed to repo
+-- outputs/
|   +-- project_summary.json      <- machine-readable results record
+-- config.py                     <- all constants (BBOX, periods, features)
+-- requirements.txt
+-- README.md
```
...
```bash
git clone https://github.com/yourusername/forest-carbon-little-andaman.git
cd forest-carbon-little-andaman
